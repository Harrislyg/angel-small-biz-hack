...
class Overview extends Component {
  render() {
    let statsItems = [
      ...
      { title: 'Outflows', currency: 'US$', amount: 3000, identifier: 'outflows' },
      { title: 'Your Balance', currency: 'US$', amount: 3000, identifier: 'your_balance' },
      { title: 'Your Target', currency: 'US$', amount: 3000, identifier: 'your_target' },
    ]
    ...
    let statsNodes = statsItems.map((statsData, index) => {
      return (
        <div className="overview__stats--item" key={index} id={`overview__stats--${statsData.identifier}`}>
          ...
          <div className="overview__stats--amount">{statsData.amount}</div>
        </div>
      );
    });
